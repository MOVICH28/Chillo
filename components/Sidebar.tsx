        ...
        const inner = (
          <>
            ...
            {cat.available && count > 0 && (
              <span className={`text-xs px-1.5 py-0.5 rounded-full font-mono ${isActive ? "bg-brand/20 text-brand" : "bg-surface-3 text-muted"}`}>
                {count}
              </span>
            )}
            {!cat.available && (
              <span className="text-[9px] px-1.5 py-0.5 rounded bg-surface-3 text-muted">soon</span>
            )}
          </>
        );

        if (!cat.available) {
          return <button key={cat.id} disabled className={cls}>{inner}</button>;
        }
